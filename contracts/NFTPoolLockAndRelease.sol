...
contract NFTPoolLockAndRelease is CCIPReceiver, OwnerIsCreator {
    ...
    // 源联上的NFT锁定到资产池子中，然后调用CCIP发送消息到目标链，发送的消息包括新owner地址和tokenId
    function lockAndSendNFT(
        uint256 tokenId,  //被锁的tokenId
        address newOwner,   //目标链新NFT的拥有者
        uint64 chainSelector,   //表示目标链
        address receiver    //目标链收获人的地址
    ) public returns (bytes32) {
        // transfer NFT to this address to lock the NFT 转移NFT到这个地址来锁定NFT
    ...
    /// handle a received message 接收ccip发来的消息
