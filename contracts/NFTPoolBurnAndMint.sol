...
//         uint64 _destinationChainSelector, //要发送的链
//         address _receiver,
//         bytes memory _text
//     )
//         internal
...
//     /// handle a received message  接受到源联发送来的信息，即：源链的合约调中的evm2AnyMessage发来的信息
...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    function burnAndSendNFT(
        uint256 tokenId,  //被锁的tokenId
        address newOwner,   //目标链新NFT的拥有者
        uint64 chainSelector,   //表示目标链
        address receiver    //收获人的地址w
    ) public returns (bytes32) {
        // transfer NFT to this address to lock the NFT 转移NFT到这个合约（指NFTPoolBurnAndMint合约）
        wnft.transferFrom(msg.sender, address(this), tokenId);

        // burn NFT befor send to ccip
        wnft.burn(tokenId);

        //constant data to be sent, 这里abi.encode(tokenId, newOwner): 对要传输的内容（tokenId和newOwner）进行编码
